...
What to tell a customer, and how to read what comes back: `customer-boot-debugging.md`. This section is the mechanism.

**Hold P while turning on** opens the debug menu on the standard ISO. Two layers: a GRUB `--hotkey=p` entry in the hidden zero-timeout `grub.cfg` runs `configfile /boot/grub/purple-menu.cfg` (GRUB polls the firmware key buffer once before booting the default, and a held key sits in that buffer), and `purple_debug_if_key_held` in the initramfs hook runs the static `purple-keyheld` (EVIOCGKEY over every evdev device, `tools/purple-keyheld.c`, compiled in the golden image and copied into the initrd) for firmware that drops the key: it raises the console log level, switches the panel to the kernel console VT with dmesg replayed, and leaves `/run/purple/debug-key`, which casper-bottom treats like `purple.debug=1`. That fallback cannot add kernel arguments, so it is the verbose boot without the USB workarounds. P is the hotkey because Apple firmware acts on Shift, D, N, T, C, X and the Command combinations at power-on, and PCs grab Esc, Del and the F keys; other letters reach GRUB on both. Both ISOs share `purple-menu.cfg` (`write_purple_menu_cfg`); the debug ISO's `grub.cfg` just sources it.
...
**`PURPLE-LOG` on the stick, every live boot.** One preallocated file (12MB, `01-remaster-iso.sh`) on the stick's third partition with two fixed regions. The first 8MB is the report from `scripts/purple-diag-collect.sh`: machine model and BIOS, cmdline, whether P was held, DRM connector state, PCI devices and drivers, failed units and pending jobs, `purple-x11` status, a process list with `wchan`, casper's log, the boot, xinitrc, Xorg and power logs, the journal tail and a short dmesg tail. The last 4MB is the kernel log streamed as it happens. `purple-stick-log.service` (`scripts/purple-stick-log.py`, live boots only, starts right after udev) mounts the partition read-only once, finds the file's sectors with `FIBMAP`, unmounts, and from then on writes both regions straight into those sectors of the partition device with `fdatasync`: the report every 10s until the boot log shows first render, then every minute for ten minutes, then every ten minutes, plus once when the service gets SIGTERM at shutdown (2s collection limit); the kernel log flushed within a second until first render and every 5s after, only when new lines arrived, followed by a seam line (`newest kernel log line above`) the next flush overwrites. Nothing is ever blanked (that would be megabytes of writes during boot): text left from an earlier or longer report sits below the end-of-report marker, which labels it. Writes must not slow the machine, so each schedule adapts: a write that took t seconds holds the next one off for at least 50t (capped at 5 minutes, so a slow stick still reports), and a due write waits while `/proc/pressure/io` shows other tasks stalled on disk over 10% of the time, for at most 30s. The report's first line gives the last write times, so a slow stick shows up as a finding. No filesystem stays mounted and no FAT metadata changes, so a power cut never dirties the partition; the file's date stays the build date, and the reserved space between the report and the kernel log is empty lines (newlines, not spaces, so grep and editors treat it as blank lines rather than one huge line). The same report also lands at `/var/log/purple/diag.txt`. `install.sh` tees its stderr to `/tmp/purple-install.log`, which the report includes and which is copied to `/var/log/purple/install.log` on the installed system.

**Before systemd.** The writer lives on the squashfs, so it cannot describe a boot that never mounted it. The initramfs hook (`purple_stick_report` in `add_purple_initramfs_hooks`, `01-remaster-iso.sh`) writes its own report (casper's log plus dmesg) into the same region, in place through a brief vfat mount (`1<>` never truncates, so the file's sectors never move), when the system image is found on the stick, again right before mounting it (after any RAM copy), and at the end of casper-bottom; the kernel has FAT built in and the initramfs carries `blkid`. `purple-stick-log` replaces that text seconds later, so a report that starts with `purple-initramfs:` means boot stopped between those points. The remaining blind spot is a stick that stops accepting writes altogether, or a failure before the kernel finds the stick, where the screen is the only record.

**USB read errors (`SQUASHFS error: Failed to read block ...: -5`, `I/O error, dev loop0`).** Seen on a 2013 Vaio Pro 13 with two different sticks failing at the same block on both ports, so it was the laptop, not the media. The boot menu's second entry, "try every fix for USB drive problems", stacks every workaround so the customer boots once: `intel_iommu=off` (the 24.04 kernel turns VT-d on by default, which Ubuntu warned breaks old firmware), `xhci_hcd.quirks=0x800000` (XHCI_NO_64BIT_SUPPORT, 32-bit DMA), `usbcore.autosuspend=-1`, `pcie_aspm=off`, and `purple.toram=1`, which makes the initramfs copy the squashfs into RAM in one sequential pass before casper mounts it (`purple_squashfs_to_ram` in `add_purple_initramfs_hooks`, `01-remaster-iso.sh`; needs the squashfs plus 768M free, else it says so and reads from the stick). A copy that fails prints how many bytes were read before the error, which is the offset that broke, and the boot continues from the stick so the usual errors still show. `log_buf_len=8M` keeps the whole kernel log for the report. The "USB fix" entries in the More options submenu try one workaround each for narrowing down later. `purple-stick-log` starts right after udev rather than at multi-user so its early reports exist before a stick dies a minute into boot.

**The `PURPLEUSB` partition** (`01-remaster-iso.sh`) is a 32MB FAT volume typed as Microsoft basic data. That type is the point: Windows and macOS hide EFI System Partitions, so the report could not live on partition 2. Both desktops mount `PURPLEUSB` like a thumb drive. Since it is also what a parent sees when they plug the stick into a running computer, it carries `HOW TO START PURPLE.txt` (shut down, leave it in, turn on, F12 or Option) and an `autorun.inf` whose label makes Explorer show the drive as "Start Purple Computer". `flash-lib.sh`'s post-settle recheck compares only partitions 1 and 2, since the settle boot writes here too.

What to tell the customer: after it gets stuck, wait two minutes, hold the power button until it turns off, plug the stick into your usual computer, open the drive named `PURPLEUSB`, and email `PURPLE-LOG`. Warn them about the other partitions: Windows may ask to format a disk and a Mac may say a disk is not readable; answer Cancel or Ignore, never Format or Initialize. If the file is missing, the kernel never found the stick or the stick refuses writes: ask them to hold P while turning on (the menu's first entry shows the boot on screen) and send a video.
