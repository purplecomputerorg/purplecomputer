...
- **`PURPLE-LOG.TXT`** on the `PURPLEUSB` drive: a full report rewritten every
  5 seconds for the first five minutes, then once a minute. Machine and BIOS,
  kernel command line, failed services, what every process is waiting on,
  Purple's own logs, the journal and dmesg. It starts as soon as the kernel
  finds the stick: the initramfs writes a first version before the system is
  even mounted (and again right before mounting it, and at the end of
  casper-bottom), so a boot that never reaches Purple still leaves one. If the
  file starts with `purple-initramfs:`, boot stopped at that stage.
- **`PURPLE-KMSG.TXT`** on the same drive: the kernel log streamed live, flushed
  at least once a second early in boot. It fills the gap between two report
  snapshots, so the last seconds before a hang are in it.
- **The turn-it-off-first note** and this drive are what a parent sees when they
  plug the stick into a running Windows or Mac computer. Both files open in
  Notepad or TextEdit.
...
4. Email `PURPLE-LOG.TXT` and `PURPLE-KMSG.TXT` from that drive, plus one photo
   of the last thing on the laptop's screen.
...
What to ask for after a menu boot: the same two files plus a photo, or a video
...
- **Nothing on the drive at all:** the kernel never found the stick, or the
  stick does not accept writes. Ask for a video of the boot with the menu's
  first entry.
- **`PURPLE-KMSG.TXT` ends mid-boot** while `PURPLE-LOG.TXT` looks complete:
  the stream stopped when the stick stopped accepting writes. The last lines
  in it are the ones just before that.
...
send the same two files and a photo.
