...
# i915.enable_psr/fbc=0: stops the partial-redraw checkerboard on old Intel
# MacBook panels. Intel-only no-op elsewhere. See guides/intel-display-tuning.md
# console=tty63: route framebuffer kernel messages to a scratch VT that's never
# foregrounded, so late systemd-shutdown loop0-busy warnings don't paint the panel
