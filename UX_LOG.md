...
- **Bulleted code-panel hints**: The cycling "Try:" hints in the Art and Music code panels now group two or three short commands per line with the same "•" bullet style Play uses (e.g. "red • blue 8 • spin"), instead of one command per line. Genuinely multi-step examples stay on their own line.
- **More varied Art hints**: The Art "Try:" hints now mix more colors (blue, yellow, pink, purple, green), all four directions (up/down/left/right), both "spin" and "turn left/right", and a couple of shapes (a square via "repeat 4 forward 8 spin", a staircase via "repeat 4 right 4 up 4"), instead of leaning on red and spin.
