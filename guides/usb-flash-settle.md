...
The constraints the barriers used to enforce moved into shared primitives: settle concurrency (RAM-bounded, `boot_settle_max_jobs`) and the four-at-a-time re-verify are `flock` slot directories (`slot_acquire` in `flash-lib.sh`), and the udev exec queue is a shared/exclusive `flock` gate (`gate_writing`/`gate_ejecting` in `flash-all.sh`): a job holds it shared while writing, which pauses the queue, and takes it exclusively for its eject, the one stage that needs `udevadm settle`, so a finished drive waits only for drives still mid-write (a retry included) rather than for the whole batch. Jobs report back through result files; the parent folds them into `ST_OK`/`ST_TRIES` for the batch summary.
