...
1. **44MB of NVIDIA GPU firmware inside the initrd.** `/usr/lib/firmware/nvidia` is pulled in because nouveau is in the initrd, and it lands in the *uncompressed* early cpio, so it is 44 of the 55MB GRUB reads. Most of it is GSP firmware for Turing and later cards. This machine's pre-Fermi MCP79 opens none of it. Every machine pays this on every boot.
...
1. **Keep nouveau and its firmware out of the initrd** (IMPLEMENTED: `zzz-purple-lean-gpu` initramfs hook in `00-build-golden-image.sh`, with a fail-loudly `lsinitramfs` check after the rebuild). Design note: the hook removes the nouveau *module* along with `firmware/nvidia`, not the firmware alone. Firmware-only stripping would let nouveau probe from the initrd with its firmware missing, and on GSP-only cards (Ada and later) a failed probe is not reliably retried, which is a black screen. With the module gone, nouveau loads post-pivot from the real filesystem where the full firmware tree lives: the exact sequence this MacBook always used (nouveau at 12.6s from real root) and what `purple-wait-display`'s 15s poll exists to absorb. There is one initrd for both boot paths (`01-remaster-iso.sh:255` copies it into `/casper/initrd`), so this reasoning was checked for live boot too: squashfs carries the full firmware tree the same way.
...
1. Install, boot, time GRUB to UI (expect: MacBook5,2 ~108s from ~180s; SSD machines unchanged).
2. `lsinitramfs /boot/initrd.img | grep -E 'nouveau|nvidia'` must be empty; `ls /usr/lib/firmware/nvidia` must NOT be (firmware stays on disk).
...
4. Live-boot the same stick on an NVIDIA machine (the initrd is shared with casper): display must come up the same way.
...
6. QEMU: `test-boot.sh --mode install` still boots the installed disk image (exercises pin + fallback on standard firmware).
