...
- **Demo greeting trimmed and Art palm tree paints again**: The Play greeting is now just "Hi!" and "It's Purple Computer!" before the color, counting, and math beats, so it no longer opens with a wall of colored letter blocks. In Art, the room now starts in paint mode, which had inverted the old palm segment's Tab toggles and made the palm draw as letters; the segment is rebuilt around the current default (absolute cursor positioning, write mode only for the intro and hand-off text) so the palm paints correctly and the "Now let's go to the Music room" lines no longer collide.
