...
- **Parent Menu "Start/Stop Recording" (dev/VM only)**: A screen-recording toggle that appears under Advanced only in a VM or the dev environment (hidden on real installed hardware). Picking it starts an ffmpeg screen capture and flips the label to "Stop Recording"; it keeps the menu open so you can start, Esc out, drive Purple, then reopen and stop. Recording also finalizes automatically if you leave Purple (Exit to Bash, shutdown). Files land in `~/shared/recordings` when that folder exists (easy to pull off the VM), else the project `recordings/`, else home. The capture command is shared with `just record-manual` via `recording-setup/capture.sh`.
