...
- **Live-USB splash sets expectations on the slow first boot**: A freshly flashed USB boots noticeably slower the very first time (the flash drive does one-time background work after the big write), then runs fast on every later boot. The live boot splash ("Starting up...") now adds "First time can take a minute or two. Faster next time!" so parents read the wait as setup, not a freeze. Only on the live-USB splash (init-top and casper-bottom), not the installed-system splash, where it does not apply.
