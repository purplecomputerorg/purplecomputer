...
> **Update 2026-09-20: the slow part is inside the kernel, with its clock stopped.** Measured on the MacBookAir3,2 with a correctly built UKI install. Default boot (UKI), Option then "EFI Boot" (shim + GRUB) and Option then "Windows" (BIOS GRUB) all took about 3 minutes by stopwatch, and on the GRUB path `Starting Purple Computer...` and `EFI stub: Loaded initrd` both printed within seconds, so no loader was reading slowly. `systemd-analyze` said 3s kernel + 17s userspace. The check that broke it open: `cat /sys/class/rtc/rtc0/time; date -u` after a slow boot showed Linux's clock **2m44s behind the hardware clock**, which is the whole missing wait. dmesg names the mechanism: `clocksource: Long readout interval, skipping watchdog check: cs_nsec: 496056280 wd_nsec: 59395720840`, i.e. in one watchdog interval the TSC advanced 0.5s while the HPET advanced 59s. The TSC only stops in deep sleep states, so the idle CPU was asleep with nothing waking it, until the kernel marked the TSC unstable (3.7s kernel time) and moved to the HPET. TSC calibration itself was right (1858 MHz). A sleeping CPU wakes on any device interrupt, which explains the variance: a live boot reading a USB stick gets a stream of them, an SSD boot almost none. Booting once with `idle=poll` (never sleep) brought the boot to about 20 seconds, and so did `nolapic_timer` (tick from the HPET/PIT, not the per-CPU LAPIC timer), which is now passed to `MacBookAir3,x` and `MacBook5,2` by `purple-variants.cfg` on the live, GRUB and UKI paths. Why the LAPIC timer fails to wake the CPU is not established; the working guess is the Nvidia chipset promoting HLT to a deeper state in which that timer stops. **Consequences for everything below:** "wall clock minus `/proc/uptime`" does not measure a pre-kernel window on these machines, because uptime comes from the stopped clock; the 5,2's "pre-kernel" minutes and its unexplained ±3 minute variance are most likely this same stall (its profile below already records "TSC marked unstable, switched to hpet"), and the claims that GRUB or the firmware read at under 1 MB/s were inferred from that arithmetic, never measured directly. The initrd prune and the `search` fix were real but small. Still to confirm on hardware: the flag on the 5,2, live boot times on both, and that sleep, lid close and audio behave with it. The install in this session also failed SHA256 verification once on the Air (SMART clean, two old interface CRC errors, repeat reads stable, reinstall passed); a failed verification leaves a bootable but unfinished install with no UKI, no grown disk and no carried-over settings.
>
> **Update 2026-09-15.** A 2010 13" MacBook Air (MacBookAir3,2, SSD, 224 MB/s under Linux) showed the same frozen `EFI stub` screen after install. Two findings changed the design. First, fix 2's `(hd0,gpt2)` pin missed there: the Air's media-less SD card reader is hd0 under Apple EFI, so every boot printed `disk hd0,gpt2 not found`, paused 10 seconds (GRUB waits after a printed error before its implicit boot), then ran the `search` scan anyway. The config now derives the root partition from the device GRUB was loaded from and never pins a disk number (`guides/nvram-boot-entry.md`). Second, the read-rate floor below is GRUB's, not the firmware's: the same firmware boots macOS in seconds, and shim and GRUB themselves load promptly. Macs now boot a unified kernel image that the firmware loads directly (install.sh Layer 7), with shim + GRUB as the fallback. The ESP-staging and MODULES=dep ideas below are superseded by that. Linux itself took 17 seconds to first paint on the Air.
>
> **Status: CLOSED for now (2026-08-13).** The deterministic costs were found, fixed, shipped, and verified on hardware: the initrd carried 44MB of modules and firmware for hardware Purple can never use (fixed, 61MB to 37MB shipped), and GRUB's `search` paid ~47s probing an empty optical drive (fixed, pinned root with search fallback). What remains on this machine is **boot-to-boot variance of ±3 minutes that is uncorrelated with anything we control**: same install, same config, boots of 2:56, 4:57, 5:39, and 6:00+ were all observed, warm versus cold reboot made no difference, and the same-config variance was present (and unexplained) during the controlled experiments too. Leading candidate: the drive intermittently busy with internal maintenance (SMART offline collection, thermal recalibration) stalling the firmware's reads; the machine's dead PRAM battery is the other suspect, though menu-on boots always showed the GRUB menu promptly, which argues the firmware segment is stable. Decomposing it needs power-to-menu vs menu-to-UI pairs across several boots (`purple-boot-timing --menu on`), recorded here as the resumption point if this machine class ever matters commercially. Until then: the deterministic work is done, the residual is below software, and further optimization (ESP staging ~1:40, install-time MODULES=dep regen ~1:45, both ~1:05) is deliberately parked; the blueprint below survives. Three unrelated bugs found along the way are fixed and shipped (`daec105`).
...
Status 2026-08-13, second ISO (fix 1 v2 + fix 2), functional checks all pass on the MacBook5,2: initrd on disk 37.2MB (vs 27.7MB in the pre-build container test; delta is module-set drift from the fresh debootstrap), lean-marker grep clean, firmware on disk intact, `purple_set_root` present, Linux 32s, live boot 33s.

**Timing validation was inconclusive, and honestly so.** One boot hit 2:56, matching the ~2:50 payload prediction, but the same installed system also produced 4:57 (cold), 5:39 (warm `sudo reboot`), and 6:00+ (first boot after install, which the user journal proves was pre-kernel: sessions started at monotonic 15.5s). Warm versus cold made no difference, the USB stick was out, and nothing we ship differed between those boots. The same-config variance was also present during the experiment days (one anomalous ~30s-class boot amid multi-minute neighbors) and went unexplained then. Conclusion: this machine's pre-kernel time has a large nondeterministic component that our changes do not and cannot control. The controlled menu-to-UI measurements (180s → 155s → 108s → 66s as payload and `search` changed) remain the evidence that the deterministic fixes do what they claim.

To resume someday: `sudo purple-boot-timing --menu on`, then record power-to-menu and Enter-to-UI separately for several boots. Menu time swinging implicates the firmware (dead PRAM battery is documented on this unit); Enter-to-UI swinging implicates the drive's internal maintenance stalling firmware reads. During all experiment-day boots the menu appeared promptly, which favors the drive theory.
