#!/usr/bin/env python3
"""
Music Mode demo composition generator. Dev machine only, not part of the product.

Uses Claude to generate PlayKeys sequences from natural language prompts.
The result is checked in as a fixed demo segment that Purple simply replays.
Two-pass approach: generate, then self-review the grid visualization.

Usage:
    python tools/music_ai.py "smiley face that sounds like a nice tune"
    python tools/music_ai.py "heart" --no-review
    python tools/music_ai.py "rainstorm" --json
    python tools/music_ai.py "happy birthday" --install
"""
...
from purple_tui.music_constants import GRID_KEYS, ALL_KEYS, COLORS, NOTE_FREQUENCIES, PERCUSSION
...
def build_generate_prompt(goal: str) -> tuple[str, str]:
    ...
    system = f"""You are a composer and visual artist for Purple Computer's Music Mode.

Music Mode is a 10x4 grid mapped to the QWERTY keyboard. Each key press plays a sound AND cycles the key's color. Your job is to create compositions that look good AND sound good.
...
"""
    ...
    user = f'Create a Music Mode composition for: "{goal}"'
...
def build_review_prompt(sections: list[dict], grid_text: str) -> tuple[str, str]:
    ...
    Returns (system_prompt, user_prompt).
    """
    system = """You are reviewing a Music Mode composition for Purple Computer. You will see the computed grid visualization showing what the composition actually produces. Check for:
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Music Mode demo compositions from natural language (dev machine only; output ships as fixed data)."
    )
    ...
    if args.json:
        ...
        if args.save:
            ...
            content = f'''"""Pre-recorded Music Mode composition: {title}

Generated by: python tools/music_ai.py "{args.goal}" --save {args.save}
"""
