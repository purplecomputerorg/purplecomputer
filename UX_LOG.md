...
- **One unified Volume entry in the Parent Menu**: The three separate rows for Silent Mode, Volume Lock, and Adjust Volume are now a single "Volume" entry. Opening it shows a slider with the current level and a "Lock at this level" toggle below it. Sliding all the way down reads "Silent" instead of a numeric zero, and turning the lock on at any level (including silent) pins the kid's volume keys at that level. Space still plays a test sound at the slider's current value. The parent menu row label switches to "Volume: Silent" or "Volume: Locked" when a lock is active. Old `silent_mode: true` settings files are migrated to `volume_lock: 0` on first load.
