...
- **More varied Art hints**: The Art "Try:" hints now show short combos that each draw something (a color plus a move, or a move plus a turn) instead of bare single words like "red" or "spin" that do nothing visible on their own. They mix more colors (blue, yellow, pink, purple, orange, green), all four directions, and both "spin" and "turn left/right", and keep a couple of shapes (a square via "repeat 4 forward 8 spin", a staircase via "repeat 4 right 4 up 4", a loop via "repeat 36 forward 5 spin") for the more advanced end.
