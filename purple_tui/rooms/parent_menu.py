...
class ParentMenu(PurpleModal):
    ...
    def _open_secret_menu(self) -> None:
        def on_result(result):
            if result in ("doodle", "photo"):
                ...
                self.dismiss()
                painter = paint_doodle if result == "doodle" else paint_photo
    ...
    def _run_shell(self) -> None:
        ...
        with self.app.suspend_with_terminal_input():
            ...
            print()
            if is_debug():
                print("Display checkerboard? Run:")
                print("  /opt/purple/scripts/debug-display.sh             (state + verdict)")
                print("  /opt/purple/scripts/debug-display.sh repro       (reproduce it)")
                print("  /opt/purple/scripts/debug-display.sh compositor off|on  (A/B the fix)")
                print()
            print("-" * 60)
            print()
            sys.stdout.flush()
